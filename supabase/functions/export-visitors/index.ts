...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!orgId)
      return new Response("Organization not specified", {
        status: 400,
        headers: corsHeaders,
      });
    ...
    const filename = `Visitors_${timestamp}.${
    ...
    if (format === "csv") {
      const csvRows = [
        ...
        ...visitors.map((v) =>
          fields
            .map((f) => {
              if (f === "city") return v.cities?.name ?? "";
              if (f === "state") return v.cities?.state_name ?? "";
              if (f === "country") return v.cities?.country_name ?? "";
              return v[f] ?? "";
            })
            .join(",")
        ),
    ...
    if (format === "xlsx") {
      ...
      const wsData = visitors.map((v) => {
        ...
        fields.forEach((f, i) => {
          if (f === "city") row[titles[i]] = v.cities?.name ?? "";
          else if (f === "state") row[titles[i]] = v.cities?.state_name ?? "";
          else if (f === "country")
            row[titles[i]] = v.cities?.country_name ?? "";
          else row[titles[i]] = v[f] ?? "";
        });
        return row;
      });
